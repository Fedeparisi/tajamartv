...
class DisneyLayout extends ConsumerStatefulWidget {
  const DisneyLayout({super.key});

  @override
  ConsumerState<DisneyLayout> createState() => _DisneyLayoutState();
}

class _DisneyLayoutState extends ConsumerState<DisneyLayout> {
        ...
        data: (channels) {
          ...
          return ListView(
            children: [
              ...
              // Disney Style Category Buttons
              _buildDisneyCategoryButtons(categories),
              const SizedBox(height: 24),
  ...
  Widget _buildDisneyCategoryButtons(List<String> categories) {
    // Tomamos las primeras 5 o menos categorías principales
    final topCategories = categories.take(5).toList();
    if (topCategories.isEmpty) return const SizedBox();
        ...
        children: topCategories.map((cat) {
          return Expanded(
            child: Padding(
              ...
              child: _DisneyCategoryButton(categoryName: cat),
            ),
          );
        }).toList(),
      ),
...
class _DisneyCategoryButton extends StatefulWidget {
  final String categoryName;
  const _DisneyCategoryButton({required this.categoryName});

  @override
  State<_DisneyCategoryButton> createState() => _DisneyCategoryButtonState();
}

class _DisneyCategoryButtonState extends State<_DisneyCategoryButton> {
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            ...
            colors: _isHovered 
              ? [const Color(0xFF333344), const Color(0xFF111122)]
              : [const Color(0xFF222233), const Color(0xFF090911)],
          ),
          border: Border.all(
          ...
          boxShadow: _isHovered
              ? [BoxShadow(color: Colors.white24, blurRadius: 15, spreadRadius: 2)]
